                        ...
                        foreach (var attribute in contentMeta.Attributes.Where(ca => !ca.Alias.Contains(' ')))
                        {
                            switch (attribute.TypeName)
                            {
                                ...
                                case "DateTime":
                                    ...
                                    break;
                                case "Relation":
                                    //TODO: Нет фильтров второго уровня (по какому-то полю из связи o2m)
                                    AddSimpleFiltersForField<IntGraphType>(filterType, filterDefinitions, attribute);
                                    if (attribute.RelatedO2mContentId.HasValue)
                                    {
                                        AddContainsFiltersForField<DecimalGraphType>(filterType, filterDefinitions, attribute);
                                    }
                                    else
                                    {
